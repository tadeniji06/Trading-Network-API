  ...
  try {
    ...
    for (const [coinId, orders] of Object.entries(ordersByCoin)) {
      try {
        ...
        if (!currentPrice) {
          console.log(
            `Failed to get price for ${coinId}, skipping orders`
          );
          continue;
        }
...
const processOrder = async (order, currentPrice) => {
  try {
    ...
    console.log(
      `Executing ${order.orderType} ${order.type} order for ${order.coinSymbol}`
    );
    ...
    if (order.type === "sell") {
      ...
      if (!holding || holding.quantity < order.quantity) {
        console.log(
          `Insufficient ${order.coinSymbol} for sell order ${order._id}`
        );
        order.status = "failed";
        await order.save();
    ...
    // Inside your processOrder function, after a successful execution:
    // Add this after the order is successfully executed
    socketService.emitToUser(user._id.toString(), "order-executed", {
      orderId: order._id,
      status: "completed",
      price: currentPrice,
      total: order.total,
    ...
    // Also emit to all users watching this coin
    socketService.emitToCoin(order.coinId, "price-update", {
      coinId: order.coinId,
      price: currentPrice,
      timestamp: Date.now(),
    });
